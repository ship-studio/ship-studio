...
Harbr can run as a server you reach from a browser instead of as a desktop
...
Harbr's backend was written for a desktop app, where the only user is the
...
2. **Loopback by default.** The server binds `127.0.0.1` unless you set
   `HARBR_ALLOW_EXTERNAL_BIND=1`, and it logs a warning when you do.
...
5. **Preview ports are not authenticated.** Your project dev servers are proxied
   on their own ports (see `HARBR_PREVIEW_PORT_RANGE`). Anyone who can reach those
...
|---|---|---|
| `HARBR_AUTH_TOKEN` | **required** | Shared secret exchanged for a session cookie. Minimum 16 characters. The server refuses to start without it. |
| `HARBR_BIND` | `127.0.0.1:1420` | Listen address. |
| `HARBR_ALLOW_EXTERNAL_BIND` | unset | Set to `1` to permit a non-loopback `HARBR_BIND`. Required opt-in. |
| `HARBR_PUBLIC_ORIGIN` | unset | The origin the browser uses, e.g. `https://ship.example.com`. Used for CSRF and WebSocket origin checks, and enables the `Secure` cookie flag when it is `https://`. Falls back to same-origin checking against the `Host` header. |
| `HARBR_STATIC_DIR` | `dist` | Directory holding the built frontend. |
| `HARBR_PREVIEW_PORT_RANGE` | `3100-3130` | Inclusive port range preview and static-file servers allocate from. Bounded so a container can publish exactly this range. |
| `HARBR_PREVIEW_BIND` | `127.0.0.1` | Address preview listeners bind to. `0.0.0.0` inside a container. |
| `HARBR_PREVIEW_URL_TEMPLATE` | unset | How the **browser** reaches a preview listener, e.g. `https://preview-{port}.example.com`. Must contain `{port}` and start with a scheme; the server refuses to start otherwise. Unset means the browser dials `http://<host of HARBR_PUBLIC_ORIGIN>:<port>` directly. **Required when the app is served over HTTPS** — see below. |

Generate a token with something like:

```bash
export HARBR_AUTH_TOKEN="$(openssl rand -hex 32)"
```
...
pattern with `HARBR_PREVIEW_URL_TEMPLATE`. A per-port *origin* (rather than a
...
```bash
export HARBR_PREVIEW_URL_TEMPLATE="https://preview-{port}.example.com"
```
...
cargo run --manifest-path src-tauri/Cargo.toml \
  --features web --bin harbr-server
```

The host also needs the tools Harbr drives but does not install:
...
- `POST /api/login` with `{"token": "..."}` sets an `HttpOnly`,
  ...
  Rotating `HARBR_AUTH_TOKEN` therefore invalidates every outstanding session.
...
- Native window controls and deep links
- Desktop update checks and process-exit controls
- Native screenshot capture and mobile-device previews
- Native file pickers and reveal-in-file-manager actions
