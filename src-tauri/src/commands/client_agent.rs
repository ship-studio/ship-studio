...
//!
//! Manages the Node.js sidecar process that runs the Anthropic Agent SDK-based
...
pub async fn start_client_agent(
    ...
    tracing::info!(
        "Starting client agent sidecar for window {} (project: {}, hitl: {:?})",
        window_label,
        project_path,
        hitl_enabled
    );
    ...
    let mut child = Command::new(&node_path)
        ...
        // OpenRouter integration — Agent SDK passes these to the Claude Code subprocess
    ...
    // Send initialize RPC with project settings (API key goes via env vars)
...
pub fn resume_generation(window_label: String, approved: bool) -> Result<(), String> {
    ...
    tracing::info!(window_label = %window_label, "resume RPC sent to sidecar");
    Ok(())
}
