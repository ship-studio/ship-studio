...
//!
//! Ship Studio is single-user software. There are no accounts — one shared
//! `SHIP_AUTH_TOKEN` is exchanged at `POST /api/login` for an HMAC-signed
...
pub async fn require_auth(
    ...
    let is_safe_method = request.method().is_safe();
    if !is_safe_method && !origin_allowed(headers, state.config.public_origin.as_deref()) {
