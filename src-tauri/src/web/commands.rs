...
mod tests {
    ...
    async fn a_result_value_is_serialized_into_the_envelope() {
        // `get_default_agent_id` takes no arguments and returns an optional
        // string — a short path proving a success value survives serialization.
        let command = get("get_default_agent_id").expect("registered");
        let value = (command.handler)(serde_json::Value::Null)
            .await
            .expect("succeeds");
        assert!(value.is_null() || value.is_string(), "expected an optional id, got {value}");
    }
