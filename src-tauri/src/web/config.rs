...
/// Minimum accepted `HARBR_AUTH_TOKEN` length. The token is the only thing
...
impl Config {
    ...
    pub fn from_env() -> Result<Self, String> {
        let auth_token = std::env::var("HARBR_AUTH_TOKEN")
            .map_err(|_| "HARBR_AUTH_TOKEN is not set. Refusing to start: this server exposes shell access and must not run unauthenticated.".to_string())?;

        if auth_token.trim().len() < MIN_TOKEN_LEN {
            return Err(format!(
                "HARBR_AUTH_TOKEN must be at least {MIN_TOKEN_LEN} characters. Refusing to start."
            ));
        }

        let bind_raw = env_or("HARBR_BIND", DEFAULT_BIND);
        let bind = bind_raw
            .to_socket_addrs()
            .map_err(|e| format!("HARBR_BIND `{bind_raw}` is not a valid address: {e}"))?
            .next()
            .ok_or_else(|| format!("HARBR_BIND `{bind_raw}` resolved to no address"))?;

        check_bind_allowed(&bind, env_flag("HARBR_ALLOW_EXTERNAL_BIND"))?;

        let preview_ports = match std::env::var("HARBR_PREVIEW_PORT_RANGE") {
            Ok(raw) => parse_port_range(&raw)?,
            Err(_) => DEFAULT_PREVIEW_PORTS,
        };

        let preview_bind: IpAddr = env_or("HARBR_PREVIEW_BIND", "127.0.0.1")
            .parse()
            .map_err(|e| format!("HARBR_PREVIEW_BIND is not a valid IP address: {e}"))?;

        let preview_url_template = std::env::var("HARBR_PREVIEW_URL_TEMPLATE")
            .ok()
        ...
        Ok(Config {
            ...
            auth_token,
            bind,
            public_origin: std::env::var("HARBR_PUBLIC_ORIGIN")
                .ok()
                .map(|s| s.trim_end_matches('/').to_string())
                .filter(|s| !s.is_empty()),
            static_dir: PathBuf::from(env_or("HARBR_STATIC_DIR", "dist")),
            preview_ports,
            preview_bind,
...
fn check_bind_allowed(bind: &SocketAddr, allow_external: bool) -> Result<(), String> {
    ...
    Err(format!(
        "HARBR_BIND `{bind}` is not a loopback address. Set HARBR_ALLOW_EXTERNAL_BIND=1 to confirm you intend to expose this server, and put TLS in front of it."
    ))
}
...
fn validate_preview_url_template(template: &str) -> Result<(), String> {
    if !template.contains("{port}") {
        return Err(format!(
            "HARBR_PREVIEW_URL_TEMPLATE `{template}` must contain `{{port}}`"
        ));
    }
    if !template.starts_with("http://") && !template.starts_with("https://") {
        return Err(format!(
            "HARBR_PREVIEW_URL_TEMPLATE `{template}` must start with http:// or https://"
        ));
    }
    Ok(())
}
...
fn parse_port_range(raw: &str) -> Result<(u16, u16), String> {
    let (lo, hi) = raw
        .split_once('-')
        .ok_or_else(|| format!("HARBR_PREVIEW_PORT_RANGE `{raw}` must look like `3100-3130`"))?;
    let lo: u16 = lo
        .trim()
        .parse()
        .map_err(|e| format!("HARBR_PREVIEW_PORT_RANGE start is not a port: {e}"))?;
    let hi: u16 = hi
        .trim()
        .parse()
        .map_err(|e| format!("HARBR_PREVIEW_PORT_RANGE end is not a port: {e}"))?;
    if lo == 0 || hi < lo {
        return Err(format!(
            "HARBR_PREVIEW_PORT_RANGE `{raw}` is empty or inverted"
        ));
    }
    Ok((lo, hi))
}
...
/// log the user out, and rotating `HARBR_AUTH_TOKEN` *should* invalidate every
...
fn derive_session_key(token: &str) -> [u8; 32] {
    let mut mac = HmacSha256::new_from_slice(b"harbr/session-cookie/v1")
...
mod tests {
    ...
    fn env_flag_only_accepts_explicit_affirmatives() {
        ...
        let key = "HARBR_TEST_FLAG_AFFIRMATIVE";
