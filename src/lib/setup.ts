...
/** Linux. Gates the detect-only setup flow: Ship Studio never installs system
...
/**
 * Absolute path of the interactive shell Ship Studio spawns for raw terminals.
 *
...
export async function installPackages(packages: string[]): Promise<void> {
  if (isLinux()) {
    ...
    throw new Error(
      "Ship Studio doesn't install system packages on Linux. Install them with your distribution's package manager, then re-check."
    );
  }
  if (isWindows()) {
...
/**
 * Example install commands for the tools Ship Studio detects but never installs
 * on Linux.
 *
...
/**
 * The manual-install hint for an item, or `null` when Ship Studio installs it
